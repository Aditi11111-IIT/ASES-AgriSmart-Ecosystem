...
# --- 3. DATA ENGINES (STRICTLY PRESERVED) ---
@st.cache_data
def load_agri_data():
    crops = {
...
if tab == "🏠 Dashboard":
    ...
    if all_crops:
        ...
        with col_c1:
            ...
            st.info(f"Rate: **₹{base_price} / Q**")
        with col_c2:
            weight = st.number_input("Enter Quantity (Q)", min_value=0.1, value=10.0)
        ...
        if st.button("📓 Save to Agri Khata"):
            add_entry("Income (Sale)", selected_crop, weight, total_value, season_sel)
    ...
    if not df_ledger.empty:
        ...
        m1.metric("Revenue", f"₹{income:,.2f}")
        m2.metric("Investment", f"₹{expense:,.2f}", delta_color="inverse")
        m3.metric("Net Profit", f"₹{income - expense:,.2f}")

        st.dataframe(df_ledger, use_container_width=True)
        
        with st.expander("➕ Add Manual Farm Expense"):
            c1, c2, c3 = st.columns(3)
            ex_item = c1.text_input("Expense Item")
            ex_amt = c2.number_input("Amount (₹)", min_value=0)
            ex_season = c3.selectbox("Season", ["Kharif", "Rabi", "Zaid"], key="khata_ex")
            if st.button("Save Expense Record"):
                add_entry("Expense (Cost)", ex_item, "N/A", ex_amt, ex_season)
                st.rerun()
        
        csv = df_ledger.to_csv(index=False).encode('utf-8')
        st.download_button("📥 Export CSV", data=csv, file_name="agri_ledger.csv")
    else:
        st.info(f"No records found for {filter_season}.")
