...
menu = st.sidebar.radio("Go To:", ["🏠 Dashboard", "🏪 Rental & Supply Hub", "🏛️ Govt Schemes", "📚 Knowledge Hub", "📒 Agri Khata"])
...
# --- MODULE 4: KNOWLEDGE HUB ---
elif menu == "📚 Knowledge Hub":
    st.title("📚 Resource Library")
    df_k = pd.DataFrame({"Crop": ["Wheat", "Rice", "Cotton"], "N-P-K Ratio": ["120:60:40", "100:60:40", "100:50:50"], "Sowing Month": ["Nov", "June", "May"]})
    st.table(df_k)
    
    if st.button("Generate Offline PDF Report"):
        pdf_data = export_as_pdf("Agri-Knowledge-Chart", df_k)
        st.download_button("📥 Download PDF", pdf_data, "agri_report.pdf", "application/pdf")
...
# --- MODULE 5: AGRI KHATA ---
    ...
    with st.form("ledger_form"):
        ...
        if st.form_submit_button("Add Entry"):
            st.session_state.ledger = pd.concat([st.session_state.ledger, pd.DataFrame([{"Item": item, "Cost": cost}])])
